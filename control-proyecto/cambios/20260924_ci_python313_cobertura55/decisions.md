...
La ejecución remota del commit inicial falló en el escaneo de secretos en Ubuntu: el escáner leyó `.git/FETCH_HEAD`, marcó como posible secreto un ejemplo ficticio en `SECURITY-THREATS.md` y volvió a reportar hashes públicos del manifest porque comparaba rutas con separadores distintos. La corrección excluye `.git` explícitamente en el runner y la auditoría de seguridad, normaliza separadores al comparar con el baseline y reemplaza el ejemplo con texto descriptivo. El runner local completo posterior pasó en Python 3.13.12: 485 pruebas aprobadas, 60.61% de cobertura de líneas, 43.12% de ramas y 56.60% combinada (sobre el piso de 55%), además de formato, lint, tipos, Bandit, escaneo de secretos y whitespace. `scripts/security-audit.sh` pasó 8/8 controles y pre-commit pasó para los cinco archivos modificados. La revisión independiente de estos cinco archivos no encontró bloqueos técnicos ni una reducción de detección. El CI remoto del commit correctivo `a25ab2d` pasó en Ubuntu el 2026-09-24 (ejecución 36057137590), incluyendo el runner local y la carga de cobertura.
