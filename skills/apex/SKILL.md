...
7. For every generated or changed Oracle SQL file, delegate to `oracle-data-change-governance-final` and run its `validate_sql_style.py`; a `STYLE_FAIL` blocks handoff. Record decisions, plans, evidence, and limitations in the project workspace when the request creates or changes artifacts.
