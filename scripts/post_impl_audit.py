...
def generate_l1_report(results: List[AuditResult], root: Path) -> Path:
		...
		if not r.passed:
			for f in r.findings:
				...
				findings_table.append(
					f"| H-{h_seq:03d} | {severity} | {control} | {f} | | ABIERTO |"
				)
				h_seq += 1
	...
	for r in results:
		...
		controls_table.append(
			f"| {control_id} | {r.name} | {r.status} | {finding_count} hallazgo(s) |"
		)
...
def _update_registry(
	registry_path: Path, audit_id: str, now: datetime,
	revision: str, auditor: str, resultado: str,
	finding_count: int, report_name: str,
):
