...
class TypeHintCoverageTest(CodeQualityTestCase):
	...
	def test_scripts_have_type_hints(self) -> None:
		...
		for script in scripts:
			...
			self.assertTrue(
				has_hints,
				f"{script.name}: Missing type hints"
			)

	def test_main_scripts_have_docstrings(self) -> None:
		"""Test that main functions have docstrings."""
		scripts = self.get_python_scripts()
		important_scripts = [s for s in scripts if s.name in [
			"apex_export_utilities.py", "apex_metadata.py", "cli_utils.py"
		]]
		...
		for script in scripts:
			...
			if public_functions:
				...
				self.assertGreater(
					docstring_count,
					0,
					f"{script.name}: No docstrings found"
				)
	...
	def test_no_trailing_whitespace(self) -> None:
		...
		for script in scripts:
			...
			self.assertEqual(
				...
				0,
				f"{script.name}: Lines with trailing whitespace"
			)
	...
	def test_functions_not_too_complex(self) -> None:
		...
		for script in scripts:
			...
			self.assertLess(
				max_indent,
				40,
				f"{script.name}: Functions possibly too deeply nested"
			)
